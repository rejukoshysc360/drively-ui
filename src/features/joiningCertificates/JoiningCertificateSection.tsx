...
import JoiningCertificateRequestForm from "./JoiningCertificateRequestForm";
import EmployeeJoiningCertificates from "./JoiningCertificatesSelf";
import { useCan } from "../../utils/permissions";

export default function EmployeeJoiningCertificateSection() {
  const [activeTab, setActiveTab] = useState<"request" | "mycerts">("request");
  const can = useCan();

  // Adjust permission checks if needed
  const canRequest = true; // or can("joining_certificate:create_own_record_only");
  const canView = true; // or can("joining_certificate:view_own_record_only");
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] bg-gray-50 text-center rounded-md border border-gray-200 p-10">
        ...
        <p className="text-gray-500 text-sm">
          You don’t have permission to manage joining certificates.
        </p>
      </div>
    );
  }
    ...
    <div className="p-4 sm:p-6 bg-gray-50 rounded-md shadow-sm space-y-4">
      ...
      <div className="mb-6 sm:mb-8">
        ...
        <p className="text-slate-600 mt-1 text-sm sm:text-base">
          Request and download your joining certificates
        </p>
      </div>
      ...
      <div className="min-h-[400px]">
        {activeTab === "request" && canRequest && <JoiningCertificateRequestForm />}
        {activeTab === "mycerts" && canView && <EmployeeJoiningCertificates />}
      </div>
    </div>
  );
}
