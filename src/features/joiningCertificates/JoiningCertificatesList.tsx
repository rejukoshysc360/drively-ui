...
import {
  ...
  useGenerateJoiningCertificate,
  useDeleteJoiningCertificate,
  useReleaseJoiningCertificate,
  useGetJoiningCertificatePresignedURL,
} from "./hooks";
import JoiningCertificatePreviewModal from "./JoiningCertificatePreviewModal";
...
export default function JoiningCertificatesList() {
  const can = useCan();
  ...
  const generate = useGenerateJoiningCertificate();
  const releaseCert = useReleaseJoiningCertificate();
  const deleteCert = useDeleteJoiningCertificate();
  const getPresigned = useGetJoiningCertificatePresignedURL();
  ...
  // Fetch joining certificates
  const { data, isFetching, isLoading } = useJoiningCertificates({
    page,
    limit,
  ...
  const handleDownloadFromS3 = async (certificateId: string) => {
    try {
      ...
      link.href = blobUrl;
      link.download = `joining-certificate-${certificateId}.pdf`;
      ...
      toast.error("Failed to download joining certificate");
    } finally {
  ...
  const handleGenerate = async () => {
    ...
    await generate.mutateAsync({ employee_id: selectedEmployee.value });
    toast.success("Joining certificate created successfully");
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="mb-2">
        <h1 className="text-2xl font-bold flex items-center gap-2 text-gray-800">
          ...
          <span>Joining Certificates</span>
        </h1>
        <p className="text-gray-500 text-sm">
          Generate, approve, and manage employee joining certificates.
        </p>
      </div>
      ...
      {previewOpen && previewCertificate && (
        <JoiningCertificatePreviewModal
          open={previewOpen}
          onClose={() => {
      ...
      <FormDialog
        open={generateDialog}
        title="Generate Joining Certificate"
      ...
      <ConfirmDialog
        open={!!deleteTarget}
        title="Delete Joining Certificate"
        description={`Are you sure you want to delete this joining certificate for "${deleteTarget?.certificate_json.employee_name}"? This cannot be undone.`}
        confirmLabel="Delete"
        danger
