// src/payroll/certificates/SalaryCertificateRequestsWidget.tsx
import { useState } from "react";
import { FileText, Calendar, ExternalLink } from "lucide-react";
import { format } from "date-fns";
import { useNavigate } from "react-router-dom";
import { useSalaryCertificateRequests } from "./hooks";
import { APP_CONFIG } from "../../config/appConfig";

export default function SalaryCertificateRequestsWidget() {
  const navigate = useNavigate();
  const [page, setPage] = useState(1);
  const limit = APP_CONFIG.PAGE_SIZE;

  const { data, isLoading } = useSalaryCertificateRequests(page, limit, "requested");
    ...
    <div className="bg-white shadow-sm border border-gray-200 rounded-xl p-4">
     ...
     <div className="flex items-center gap-3">
      <FileText className="w-5 h-5 text-indigo-600" />
        ...
        <button
          onClick={() => navigate("/manage-salary-certificates")}
      ...
      {isLoading ? (
        ...
        <div className="text-sm text-gray-500 py-4 text-center">
          No salary certificate requests found.
        </div>
      )}
    </div>
  );
